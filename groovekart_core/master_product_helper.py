...
from . import tasks
from .models import GrooveKartProduct, GrooveKartStore
...
def map_variants(parent, product_data, mapping, store=None):
    ...
    for variant in product_data.get('variants'):
        ...
        variant['compare_at_price'] = float(compare_at_price) if compare_at_price else None
        variant['price'] = float(price) if price else None
        variants.append(variant)
    return variants


class GrooveKartMasterProductHelper(MasterProductHelperBase):
    product_model = GrooveKartProduct
    store_model = GrooveKartStore

    def __init__(self, product_id=None):
        if product_id:
            self._product = (GrooveKartProduct.objects
    ...
    def create_new_product(self, user, store_id, parent, override_fields=None, publish=False):
        ...
        self.product = GrooveKartProduct.objects.get(id=result.get('product', {}).get('id'))
    ...
    def get_variants(self):
        if self.product.source_id:
            ...
            variants_list = []
            for variant in product_data.get('variants'):
                item = variant.get('description')
                variants_list.append({'title': item, 'image': variant.get('image', {}).get('url', '')})
        else:
