    ...
    def start_command(self, *args, **options):
        ...
        for order in orders:
            try:
                ...
                data = shopify_orders_tasks.get_order_info_via_api(order, order.source_id, order.store.id)
                if data:
                    new_tracking_number = (data.get('tracking_number') and data.get('tracking_number') not in order.source_tracking)
                    if data.get('tracking_number') != '' and new_tracking_number:  # No New Tracking number to add
