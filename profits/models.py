...
INITIAL_DATE = arrow.get('2019-01-01')


class FacebookAccess(models.Model):
    class Meta:
        index_together = ['store_content_type', 'store_object_id']

    store_content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    store_object_id = models.PositiveIntegerField()
    store = GenericForeignKey('store_content_type', 'store_object_id')

    user = models.ForeignKey(User, related_name='facebook', on_delete=models.CASCADE)
    ...
    def _reload_api(self):
        self._api = FacebookAdsApi.init(
            ...
            self.access_token,
            api_version=settings.FACEBOOK_MARKETING_API_VERSION
        )
...
class FulfillmentCost(models.Model):
    class Meta:
        ordering = ['-created_at']
        index_together = ['store_content_type', 'store_object_id', 'order_id', 'source_id']
...
class OtherCost(models.Model):
    class Meta:
        index_together = ['store_content_type', 'store_object_id', 'date']
        ordering = ['-date']
