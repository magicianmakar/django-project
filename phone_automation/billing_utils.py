...
class CallflexOveragesBilling:
    ...
    def update_overages(self):
        ...
        if phonenumber_usage_tollfree['total'] is not False and phonenumber_usage_tollfree['used'] >= phonenumber_usage_tollfree['total']:
            overage_phones = self.user.twilio_phone_numbers.filter(type="tollfree").exclude(status='released'). \
                all()[safe_int(phonenumber_usage_tollfree['total']):]
            # going thru each "overage" number and calculating monthes passed (after provisioining or subscription start, which was first )
        ...
        if phonenumber_usage_local['total'] is not False and phonenumber_usage_local['used'] >= phonenumber_usage_local['total']:
            overage_phones = self.user.twilio_phone_numbers.filter(type="local").exclude(status='released'). \
                all()[safe_int(phonenumber_usage_tollfree['total']):]
