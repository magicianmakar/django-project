(function(product_id, variants_mapping, store_options, store_variants) {
    ...
    $('#save-mapping').click(function(e) {
        ...
        $.ajax({
            url: api_url('variants-mapping', 'fb_marketplace'),
            type: 'POST',
            data: mapping,
            context: {
                btn: $(this)
        ...
        var render_options = function(response) {
            ...
            $.each(supplier_options, function(index, supplier_option) {
                ...
                optionEl.find('.store-option-select').change(function() {
                    ...
                    if (mapped >= 0) { // The supplier option was mapped to one fb_marketplace option
                    ...
                    // Disable mapped fb_marketplace options in other dropdowns
                    var values = [];
    ...
    $('#save-options-mapping').click(function(e) {
        ...
        var validateMapping = function(response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                if (mapped) { // all fb_marketplace option values should be mapped to one supplier option value
                    mapped = parseInt(mapped);
                    var store_values = store_options[mapped]['values'];
                    var mapping = {
                        mapped: mapped,
