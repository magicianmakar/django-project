(function(store_id, product_id, fb_marketplace_product, bundle_mapping) {
    'use strict';
    ...
    Vue.component('bundle-mapping-table', {
        ...
        methods: {
            saveChanges: function(e) {
                ...
                $.ajax({
                    url: '/api/fb_marketplace/bundles-mapping',
                    type: 'POST',
                    data: JSON.stringify({
                        mapping: api_data,
                        product: product_id,
    ...
    Vue.component('add-product-row', {
        ...
        methods: {
            showSelection: function(e) {
                ...
                if (this.initSelect) {
                    $('.product-select', this.$el).click(function (e) {
                        $('#modal-fb_marketplace-product').prop('connected', true);
                        $('#modal-fb_marketplace-product').modal('show');

                        // Product Connect
                        window.fb_marketplaceProductSelected = function (store, fb_marketplace_id, product_data) {
                            if (!product_data.fb_marketplace) {
                                toastr.error('Product is not connected');
                                return;
                            }

                            setupVariantSelection(product_data.fb_marketplace);
                            $v.new_product = {
                                id: product_data.fb_marketplace,
            ...
            setupVariantSelection: function(product_id) {
                ...
                $.ajax({
                    url: '/fb_marketplace/autocomplete/variants',
                    type: 'GET',
                    data: {
                        store: store_id,
                        product: product_id,
...
})(store_id, product_id, fb_marketplace_product, bundle_mapping);
