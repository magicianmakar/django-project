...
def map_variants(parent, product_data, mapping, store=None):
    ...
    for variant in product_data.get('variants'):
        ...
        if parent_variant:
            ...
            parent_variant = {'price': variant['sale_price'], 'compare_at_price': variant['regular_price']}
        else:
            parent_variant = {'price': variant['regular_price'], 'compare_at_price': None}
...
class WooMasterProductHelper(MasterProductHelperBase):
    product_model = WooProduct
    store_model = WooStore

    def __init__(self, product_id=None):
        if product_id:
            self._product = (WooProduct.objects
    ...
    def create_new_product(self, user, store_id, parent, override_fields=None, publish=False):
        ...
        self.product = WooProduct.objects.get(id=result.get('product', {}).get('id'))
    ...
    def get_variants(self):
        if self.product.source_id:
            ...
            for variant in product_data.get('variants'):
                ...
                item = ' / '.join(options)
                variants_list.append({'title': item, 'image': variant.get('image', {}).get('src', '')})
        else:
