...
$('#modal-add-variant-options .save-add-options').click(function (e) {
    ...
    if (options) {
        product.options = options.split(',').map(function(name) {
            return {name: name, option_values: []};
        });
...
$("a.add-new-variant").click(function (e) {
    ...
    var row = $('<tr>');
    row.addClass('woocommerce-variant');
    ...
    $('#woocommerce-variants tbody').append(row);
});

$('body').on('click', 'tr.woocommerce-variant td.add-variant-image', function(e) {
    e.preventDefault();
...
$('#modal-add-variant-image').on('click', '.add-variant-image-block', function(e) {
    ...
    var img = $('#woocommerce-variants tr[variant-id="' + id + '"]').find('.add-variant-image img');
...
$('body').on('click', 'tr.woocommerce-variant .delete-variant', function(e) {
    e.preventDefault();
...
$('body').on('click', 'tr.woocommerce-variant .save-variant-name', function(e) {
    e.preventDefault();
    ...
    editElement.find('input').each(function() {
        var value = $(this).val();
        if (value) {
            variant.option_values.push({label: $(this).attr('name'), value: value});
            if (title) {
                title += ' / ' + value;
            } else {
                title = value;
