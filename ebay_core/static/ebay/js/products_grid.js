...
/* global boardsMenu, sendProductToEbay, api_url, Pusher */
...
$('#board-product-send').click(function(e) {
    ...
    $.ajax({
        url: api_url('board-add-products', 'ebay'),
        type: 'POST',
        data: data,
        success: function(data) {
            if ('status' in data && data.status == 'ok') {
...
function changeBoard(board_id, options) {
    ...
    $.ajax({
        url: api_url('product-board', 'ebay'),
        type: 'POST',
        data: {
    ...
    $.contextMenu({
        ...
        callback: function(key, options) {
            if (key == 'add') {
                ...
                $('#modal-board-add').prop('store-type', 'ebay');
...
$('.ebay-product-import-btn').click(function (e) {
    e.preventDefault();

    $('#modal-ebay-product .ebay-store').val($(this).attr('store'));
    $('#modal-ebay-product').modal('show');
});

window.ebayProductSelected = function (store, ebayId) {
    $('#modal-ebay-product').modal('hide');

    $('#modal-supplier-link').prop('ebay-store', $('#modal-ebay-product .ebay-store').val());
    $('#modal-supplier-link').prop('ebay-product', ebayId);
...
$('.add-supplier-info-btn').click(function (e) {
    ...
    $.ajax({
            ...
            type: 'POST',
            data: {
                store: $('#modal-supplier-link').prop('ebay-store'),
            ...
            setTimeout(function() {
                window.location.href = '/ebay/product/' + data.product;
            }, 1500);
        }).fail(function(data) {
