...
class OrdersList(ListView):
    ...
    def get_queryset(self):
        ...
        _, limit_date = self.ebay_utils.get_orders_count_and_limit_date(self.request.user, thirty_days_ago, today)
    ...
    def get_context_data(self, **kwargs):
        ...
        if context['suredone_order_limit'] > -1:
            today = datetime.today().strftime('%Y-%m-%dT%H:%M:%S.%f')
            thirty_days_ago = (datetime.today() - timedelta(days=30)).strftime('%Y-%m-%dT%H:%M:%S.%f')
            context['suredone_orders_count'], limit_date = self.ebay_utils.get_orders_count_and_limit_date(
