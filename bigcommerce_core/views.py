...
class OrdersList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_orders'] = self.request.user.profile.get_orders_count(BigCommerceOrderTrack)
            ...
            if shipstation_address_changed:
                ...
                for pls_order_id, line_items in update_shipstation_items.items():
                    update_shipstation_address.apply_async(
                        args=[pls_order_id, self.store.id, 'bigcommerce'],
                        countdown=5
                    )
...
class OrderPlaceRedirectView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if auto_fulfill_limit != -1:
            orders_count = parent_user.profile.get_orders_count(BigCommerceOrderTrack)

            if not settings.DEBUG and not auto_fulfill_limit or orders_count + 1 > auto_fulfill_limit:
                messages.error(self.request, "Woohoo! 🎉. You are growing and you've hit your orders limit for this month. "
