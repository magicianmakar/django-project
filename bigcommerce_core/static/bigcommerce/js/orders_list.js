...
$('#fullfill-order-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Manual Fulfillment', 'BigCommerce', sub_conf.shop);

    $.ajax({
        url: api_url('fulfill-order', 'bigcommerce'),
        type: 'POST',
...
$('.filter-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Filter Toggle', 'BigCommerce', sub_conf.shop);
...
$('.save-filter-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Save Filter', 'BigCommerce', sub_conf.shop);

    $.ajax({
        url: api_url('save-orders-filter', 'bigcommerce'),
        type: 'POST',
...
function confirmDeleteOrderID(e) {
    ...
    function(isConfirm) {
        if (isConfirm) {
            ...
            ga('clientTracker.send', 'event', 'Delete Order ID', 'BigCommerce', sub_conf.shop);
        }
    });
}
...
function deleteOrderID(tr_parent, order_id, line_id) {
    ...
    function(isConfirm) {
        if (isConfirm) {
            $.ajax({
                url: api_url('order-fulfill', 'bigcommerce') + '?' + $.param({'order_id': order_id, 'line_id': line_id, }),
                type: 'DELETE',
...
function addOrderSourceRequest(data_api, callback) {
    ...
    $.ajax({
        url: api_url('order-fulfill', 'bigcommerce'),
        type: 'POST',
        data: data_api,
        context: {
            data_api: data_api
...
$('.add-order-note').click(function (e) {
    ...
    swal({
        ...
        $.ajax({
            url: api_url('order-add-note', 'bigcommerce'),
            type: 'POST',
            data: {
                'order_id': order_id,
                'store': store,
            ...
            success: function (data) {
                if (data.status == 'ok') {
                    swal.close();
                    toastr.success('Note added to the order in BigCommerce store.', 'Add Note');
                } else {
...
$('.note-panel .note-edit-save').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Edit Order Note', 'BigCommerce', sub_conf.shop);

    $.ajax({
        url: api_url('order-note', 'bigcommerce'),
        type: 'POST',
        data: {
            'order_id': order_id,
            'store': store,
        ...
        success: function (data) {
            if (data.status == 'ok') {
                toastr.success('Order Note', 'Order note saved in BigCommerce store.');
...
$('.hide-ordered-btn').click(function () {
    ...
    ga('clientTracker.send', 'event', 'Hide Ordered Click', 'BigCommerce', sub_conf.shop);
});
...
$('.hide-non-connected-btn').click(function () {
    ...
    ga('clientTracker.send', 'event', 'Hide Non-Connected Click', 'BigCommerce', sub_conf.shop);
});
...
$('.add-supplier-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Add Supplier', 'BigCommerce', sub_conf.shop);

    $('#modal-supplier-link').prop('bigcommerce-store', $(this).attr('store-id'));
    $('#modal-supplier-link').prop('bigcommerce-product', $(this).attr('bigcommerce-product'));
