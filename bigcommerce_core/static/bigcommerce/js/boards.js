$(document).ready(function() {
    ...
    $("#bigcommerce-new-board-add-form").submit(function(e) {
        e.preventDefault();
        var $title = $(this).find('input[name="title"]');
        var boardName = $title.val().trim();

        $.ajax({
            url: api_url('boards-add', 'bigcommerce'),
            type: 'POST',
            data: {title: boardName},
            success: function(data) {
                if ('status' in data && data.status == 'ok') {
                    $('#bigcommerce-modal-board-add').modal('hide');
                    $title.val('');
    ...
    $('.bigcommerce-edit-board-btn').click(function(e) {
        ...
        $.get(api_url('board-config', 'bigcommerce'), {'board_id': boardId}).done(function(data) {
            ...
            $('#bigcommerce-modal-board-update').modal('show');
        });
    });

    $('#bigcommerce-board-update-form').submit(function(e) {
        e.preventDefault();
        var data = {
        ...
        $.post(api_url('board-config', 'bigcommerce'), data).done(function() {
            $('#bigcommerce-modal-board-update').modal('hide');
            window.location.href = window.location.href;
        });
    });
    ...
    $('.bigcommerce-delete-board-btn').click(function(e) {
        ...
        swal({
            ...
            $.ajax({
                url: api_url('board', 'bigcommerce') + '?' + $.param({board_id: boardId}),
                method: 'DELETE',
                success: function(data) {
                    if ('status' in data && data.status == 'ok') {
                        swal.close();
                        ...
                        if ($row.length) {
                            ...
                            window.location.href = '/bigcommerce/boards/list';
                        }
                    } else {
    ...
    $('.bigcommerce-empty-board-btn').click(function(e) {
        ...
        swal({
            ...
            $.ajax({
                url: api_url('board-empty', 'bigcommerce'),
