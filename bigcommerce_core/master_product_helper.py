...
from multichannel_products_core.utils import apply_templates, apply_pricing_template
from shopified_core import permissions
from . import tasks
from .models import BigCommerceProduct, BigCommerceStore
from .utils import get_image_url_by_hash
...
def map_variants(parent, product_data, mapping, store=None):
    ...
    for variant in product_data.get('variants'):
        parent_variant = (list(filter(
            lambda x: x['variant'] == mapping[
                ' / '.join([item.get('label') for item in variant.get('option_values', [])])],
            parent_variants
        )))

        if parent_variant:
            parent_variant = parent_variant[0]
        elif variant.get('sale_price'):
            parent_variant = {'price': variant['sale_price'], 'compare_at_price': variant['price']}
        else:
            parent_variant = {'price': variant['price'], 'compare_at_price': None}
        ...
        variants.append({**variant, 'price': parent_variant['price'],
                         'compare_at_price': parent_variant['compare_at_price']})
    return variants


class BigCommerceMasterProductHelper(MasterProductHelperBase):
    _product = None
    ...
    def __init__(self, product_id=None):
        if product_id:
            self._product = (BigCommerceProduct.objects
    ...
    def create_new_product(self, user, store_id, parent, override_fields=None, publish=False):
        ...
        self.product = BigCommerceProduct.objects.get(id=result.get('product', {}).get('id'))
    ...
    def get_variants(self):
        if self.product.source_id:
            ...
            variants_list = []
            for variant in product_data.get('variants'):
                options = variant.get('option_values')
                item = ' / '.join([option.get('label') for option in options])
                variants_list.append({'title': item, 'image': variant.get('image_url')})
        else:
