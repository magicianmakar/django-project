...
def add_new_google_store(sd_account_id, pusher_channel, user_id, instance_id=None, event_name=None, error_message=None):
    user = User.objects.get(id=user_id)
    sd_pusher = SureDonePusher(pusher_channel)
    default_event = 'google-store-add' if event_name is None else event_name
...
def update_profile_settings(user_id, store_id, data, pusher_channel, event_name=None):
    pusher_event = event_name if event_name else 'google-advanced-settings-update'
    sd_pusher = SureDonePusher(pusher_channel)
    try:
        user = User.objects.get(id=user_id)
        ...
        resp = None
        try:
            resp = google_utils.api.update_user_settings(data)
...
def product_save(req_data, user_id, pusher_channel):
    sd_pusher = SureDonePusher(pusher_channel)
    default_event = 'google-product-save'
    ...
    try:
        ...
        if store:
            ...
            except (GoogleStore.DoesNotExist, ValueError):
                capture_exception()
                sd_pusher.trigger(default_event, {
                    'success': False,
            ...
            store = user.profile.get_google_stores().first()
        ...
        url = reverse('google:product_detail', kwargs={'pk': product_guid, 'store_index': store.pk})
...
def product_export(user_id, parent_guid, store_id, pusher_channel):
    ...
    sd_pusher = SureDonePusher(pusher_channel)
    pusher_event = 'google-product-export'
    try:
        permissions.user_can_view(user, store)
        ...
        api_response = google_utils.relist_product(variant_guids, 'google', store.store_instance_id)
        ...
        if len(api_error_messages):
            sd_pusher.trigger(pusher_event, {
                ...
                'error': google_utils.format_error_messages_by_variant(api_error_messages)
            })
        ...
        sd_pusher.trigger(pusher_event, {
            'success': True,
            'product': product.guid,
            'product_url': reverse('google:product_detail', kwargs={'pk': parent_guid, 'store_index': store.pk}),
        })
...
def product_update(user_id, parent_guid, product_data, store_id, skip_publishing, pusher_channel):
    ...
    try:
        ...
        google_cat_id = product_data.pop('google_category_id', None)
        google_prefix = google_utils.get_google_prefix(store.store_instance_id)
        if google_cat_id:
            product_data[f'{google_prefix}productcategory'] = google_cat_id

        product_data['dropifiedgoogleproductlink'] = product_data.pop('page_link', '')
        ...
        sd_api_response = google_utils.update_product_details(
            product_data, 'google', store.store_instance_id, skip_all_channels=skip_publishing,
            variants_config=json.loads(product.parsed.get('variantsconfig', []))
        )

        url = reverse('google:product_detail', kwargs={'pk': product.guid, 'store_index': store.pk})
        ...
        updated_product = google_utils.get_google_product_details(
            parent_guid,
            smart_board_sync=True,
            add_model_fields={'google_category_name': product_data.get('google_category_name', '')}
        )
        ...
        sd_pusher.trigger(pusher_event, {
            ...
            'error': http_exception_response(e, json=True).get('message', 'Server Error'),
            'product': product.guid,
            'product_url': reverse('google:product_detail', kwargs={'pk': product.guid, 'store_index': store.pk})
        })
...
def product_duplicate(user_id, parent_sku, product_data, store_id):
    ...
    try:
        result = GoogleUtils(user).duplicate_product(product_data, store)
        ...
        product_guid = result.get('parent_guid', '')
        duplicated_product_url = reverse('google:product_detail', kwargs={'pk': product_guid, 'store_index': store.pk})
...
def product_delete(user_id, parent_guid, store_id):
    ...
    try:
        ...
        err_msg = google_utils.format_error_messages('actions', sd_api_resp)
        if err_msg:
            store.pusher_trigger(pusher_event, {
                'success': False,
                'product': product.guid,
...
def get_latest_order_note_task(user_id, store_id, order_ids):
    ...
    for order_id in order_ids:
        ...
        if not have_error:
            try:
                data['success'] = True
                data['note'] = GoogleUtils(user).get_latest_order_note(order_id)
...
def variant_image(user_id, parent_guid, guid, product_data, store_id, skip_publishing=True):
    ...
    try:
        ...
        sd_api_response = google_utils.update_product_variant(product_data, skip_all_channels=skip_publishing)
        ...
        # Fetch SureDone updates and update the DB
        updated_product = google_utils.get_google_product_details(parent_guid, smart_board_sync=True)
...
def products_supplier_sync(self, store_id, user_id, products, sync_price, price_markup, compare_markup, sync_inventory, cache_key):
    store = GoogleStore.objects.get(id=store_id)
    user = User.objects.get(id=user_id)
    products = GoogleProduct.objects.filter(guid__in=products, user=store.user, store=store, source_id__gt=0)
    total_count = 0
    ...
    for product in products:
        ...
        push_data['google_link'] = product.google_url
        ...
        try:
            # Fetch google variants
            variants = product.retrieve_variants()
            variants = [variant.__dict__ for variant in variants]
        except Exception:
            push_data['status'] = 'fail'
            push_data['error'] = 'Failed to load google data'
            push_data['fail'] += 1
            store.pusher_trigger('products-supplier-sync', push_data)
            continue
        ...
        try:
            ...
            if updated:
                google_utils = GoogleUtils(user)
                try:
                    google_utils.update_product_details(
                        updated_product_data=updated_product_data,
                        store_type=store.store_type,
                        store_instance_id=store.store_instance_id
                    )
                    google_utils.get_google_product_details(parent_guid=updated_product_data['guid'])
                except Exception:
                    capture_exception()
                    push_data['status'] = 'fail'
