...
class ProductsList(ListView):
    model = GoogleProduct
    template_name = 'google/products_grid.html'
    ...
    def get_queryset(self):
        ...
        try:
            return GoogleUtils(user).get_google_products(store_id, in_store, sort=sort, request=self.request)
        except AttributeError:
            return []

    def get_context_data(self, **kwargs):
        context = super(ProductsList, self).get_context_data(**kwargs)
        context['breadcrumbs'] = [{'title': 'Products', 'url': reverse('google:products_list')}]

        # Non-connected view
        if self.request.GET.get('store', 'n') == 'n':
            context['breadcrumbs'].append({'title': 'Non Connected', 'url': f"{reverse('google:products_list')}?store=n"})

            in_store = safe_int(self.request.GET.get('in'))
            if in_store:
                in_store = get_object_or_404(GoogleStore, id=in_store, user=self.request.user.models_user)
                context['breadcrumbs'].append({'title': in_store.title,
                                               'url': f"{reverse('google:products_list')}?store=n&in={in_store.id}"})
        # Connected view
        elif self.request.GET.get('store', 'n') == 'c':
            context['breadcrumbs'].append({'title': 'Connected', 'url': f"{reverse('google:products_list')}?store=c"})

        # Connected to a specific google store view
        elif safe_int(self.request.GET.get('store')):
            store = get_object_or_404(GoogleStore, id=self.request.GET.get('store'))
            permissions.user_can_view(self.request.user, store)

            context['store'] = store
            context['breadcrumbs'].append({'title': store.title,
                                           'url': f"{reverse('google:products_list')}?store={store.id}"})
...
class ProductDetailView(DetailView):
    model = GoogleProduct
    template_name = 'google/product_detail.html'
    ...
    def dispatch(self, request, pk, store_index, *args, **kwargs):
        ...
        self.store = get_object_or_404(GoogleStore, id=store_index)
    ...
    def get_object(self, queryset=None):
        return GoogleUtils(self.request.user).get_google_product_details(self.product_guid)
    ...
    def get_context_data(self, **kwargs):
        ...
        products_url = reverse('google:products_list')
...
class ProductMappingView(DetailView):
    model = GoogleProduct
    template_name = 'google/product_mapping.html'
    context_object_name = 'product'
    product_guid = ''
    store = None
    ...
    def get_object(self, queryset=None):
        try:
            product = GoogleProduct.objects.get(guid=self.product_guid)
            return product
    ...
    def get_current_supplier(self, product: GoogleProduct):
        pk = self.request.GET.get('supplier') or product.default_supplier_id
        return product.get_suppliers().get(pk=pk) if pk else None

    def get_variants_map(self, sd_product_data: dict, product: GoogleProduct, supplier: GoogleSupplier):
    ...
    def get_context_data(self, **kwargs):
        ...
        products_list_url = reverse('google:products_list')
        context['breadcrumbs'] = [
            {'title': 'Products', 'url': products_list_url},
            {'title': product.store.title, 'url': f"{products_list_url}?store={self.store.id}"},
            {'title': product.title, 'url': reverse('google:product_detail', args=[self.store.id, product.guid])},
            'Variants Mapping',
        ]
...
class MappingSupplierView(DetailView):
    model = GoogleProduct
    # template_name = 'google/mapping_supplier.html'
    context_object_name = 'product'
    product_guid = ''
    store = None
...
class MappingBundleView(DetailView):
    model = GoogleProduct
    # template_name = 'google/mapping_bundle.html'
    context_object_name = 'product'


class VariantsEditView(DetailView):
    model = GoogleProduct
    template_name = 'google/variants_edit.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        product_url = reverse('google:product_detail',
        ...
        context = {
            ...
            'product_url': self.object.google_url,
            'page': 'product',
            'breadcrumbs': [
                {'title': 'Products', 'url': reverse('google:products_list')},
...
class OrdersList(ListView):
    model = None
    template_name = 'google/orders_list.html'
    context_object_name = 'orders'
    paginator_class = GoogleListPaginator
    _google_utils = None
    paginate_by = 20
    products = {}
    url = reverse_lazy('google:orders_list')
    bulk_queue = False
    store = None
    product_filter = None
    filters_config = {}
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.bulk_queue:
            return format_queueable_orders(context['orders'], context['page_obj'], store_type='google', request=self.request)
    ...
    def get_filters(self):
        ...
        All possible filter values:
            ...
            order_id: google order in a string type
            product: TODO
            status: TODO
            supplier: TODO
        :return:
        ...
        if product_guid:
            self.product_filter = self.google_utils.get_google_product_details(product_guid)
    ...
    def get_queryset(self):
        ...
        _, limit_date = self.google_utils.get_orders_count_and_limit_date(self.request.user, thirty_days_ago, today)
        ...
        return GoogleOrderListQuery(self.request.user, self.get_store(), self.filters_config)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['shipping_carriers'] = self.google_utils.store_shipping_carriers()
        ...
        if context['suredone_order_limit'] > -1:
            today = datetime.today().strftime('%Y-%m-%dT%H:%M:%S.%f')
            thirty_days_ago = (datetime.today() - timedelta(days=30)).strftime('%Y-%m-%dT%H:%M:%S.%f')
            context['suredone_orders_count'], limit_date = self.google_utils.get_orders_count_and_limit_date(
...
class OrdersTrackList(ListView):
    model = GoogleOrderTrack
    paginator_class = SimplePaginator
    template_name = 'google/orders_track.html'
    context_object_name = 'orders'
    paginate_by = 20
    _google_utils = None
    ...
    def get_paginate_by(self, queryset):
        custom_limit = safe_int(self.request.user.get_config('_google_order_track_limit'), None)
        if custom_limit:
    ...
    def get_queryset(self):
        ...
        orders = GoogleOrderTrack.objects.select_related('store', 'user', 'user__profile') \
            .filter(user=self.request.user.models_user, store=store) \
            .defer('data')
        if query:
            clean_query = clean_query_id(query)
            order_ids = self.google_utils.order_id_from_name(store, query, [clean_query])
        ...
        if fulfillment_filter == '1':
            orders = orders.filter(google_status='fulfilled')
        elif fulfillment_filter == '0':
            orders = orders.exclude(google_status='fulfilled')
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['orders'] = self.google_utils.get_tracking_orders(context['orders'], self.paginate_by)

            try:
                context['orders'] = self.google_utils.get_tracking_products(context['orders'], self.paginate_by)
        ...
        context['breadcrumbs'] = [{
            'title': 'Orders',
            'url': reverse('google:orders_list')
        }, {
            'title': 'Tracking',
            'url': reverse('google:orders_track')
        }, {
            'title': context['store'].title,
            'url': f"{reverse('google:orders_list')}?store={context['store'].id}"
        }]
...
class BoardsList(ListView):
    ...
    template_name = 'google/boards_list.html'
...
class BoardDetailView(DetailView):
    ...
    template_name = 'google/board.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        products = GoogleUtils(self.request.user).get_google_products(store_id=self.request.GET.get('store'),
        ...
        context['breadcrumbs'] = [{'title': 'Boards', 'url': reverse('google:boards_list')}, self.object.title]
...
class OrderPlaceRedirectView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        if not self.request.user.can('auto_order.use'):
            ...
            return '/google/orders'

        if not self.request.GET.get('SAStore'):
            return set_url_query(self.request.get_full_path(), 'SAStore', 'google')
        ...
        if self.request.GET.get('supplier'):
            supplier = get_object_or_404(GoogleSupplier, id=self.request.GET['supplier'])
        ...
        redirect_url = affiliate_link_set_query(redirect_url, 'SAStore', 'google')
        ...
        limit_check_key = f'order_limit_google_{parent_user.id}'
        if cache.get(limit_check_key) is None and plan.auto_fulfill_limit != -1:
            ...
            orders_count = parent_user.googleordertrack_set.filter(created_at__gte=month_start).count()
        ...
        if order_key:
            ...
            order_data = order_data_cache(f'google_{order_key}')
        ...
        if order_data and settings.KEEN_PROJECT_ID and not cache.get(event_key):
            try:
                store = GoogleStore.objects.get(id=store)
                permissions.user_can_view(self.request.user, store)
...
@method_decorator(PlatformPermissionRequired('google'), name='dispatch')
class ProfitDashboardView(ProfitDashboardMixin, ListView):
    store_type = 'google'
    store_model = GoogleStore
    base_template = 'base_google_core.html'
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if error:
            ...
            if error_reason == 'user_denied':
                messages.error(self.request, 'Failed to add a Google store. Please grant all required permissions '
                                             'to manage a store using Dropified.')
            else:
                messages.error(self.request, 'Failed to add a Google store. Please try again later.')

            capture_message('Error adding a Google store.', extra={
                'error': error,
        ...
        google_granted_scopes = self.request.GET.get('granted_scopes')
        google_denied_scopes = self.request.GET.get('denied_scopes')
        google_state = self.request.GET.get('state')

        google_utils = GoogleUtils(user)
        sd_api_resp = None
        try:
            sd_api_resp = google_utils.api.authorize_google_complete(
                instance=instance_id,
                code=google_code,
                granted_scopes=google_granted_scopes,
                denied_scopes=google_denied_scopes,
                state=google_state
            )
            sd_api_resp.raise_for_status()
        except HTTPError:
            capture_exception(extra={
                'description': 'API error when authorizing a Google store with SureDone.',
                'suredone_account_id': google_utils.sd_account.id,
            ...
            messages.error(self.request, 'Something went wrong when authorizing a Google store. Please try again.')
            return reverse('google:index')
        ...
        if not successful_results:
            ...
            error_message_default = 'Failed to authorize a Google store. ' \
        ...
        try:
            GoogleStore.objects.get(store_instance_id=instance_id_for_db, user=user.models_user)
        except GoogleStore.DoesNotExist:
            GoogleStore.objects.create(
                sd_account=google_utils.sd_account,
...
class CompleteAuthView(DetailView):
    ...
    template_name = 'google/shop_onboarding.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        permissions.user_can_view(self.request.user, self.object)
        context['breadcrumbs'] = ['Google Shop Onboarding']

        if self.object.auth_completed:
            error = f'The Google Store <strong>{self.object.title}</strong> has already been onboarded.'
            messages.error(self.request, error)
            return context
