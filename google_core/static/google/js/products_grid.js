...
/* global boardsMenu, sendProductToGoogle, api_url, Pusher */
...
$('#board-product-send').click(function(e) {
    ...
    $.ajax({
        url: api_url('board-add-products', 'google'),
        type: 'POST',
        data: data,
        success: function(data) {
            if ('status' in data && data.status == 'ok') {
...
function changeBoard(board_id, options) {
    ...
    $.ajax({
        url: api_url('product-board', 'google'),
        type: 'POST',
        data: {
...
$('#google-send-btn').click(function(e) {
    ...
    channel.bind('product-export', function(data) {
        ...
        if ($.inArray(productId, productIds) >= 0) {
            if (data.success) {
                toastr.success('Product sent to Google store.');
                totalSuccess += 1;
            } else if (data.error) {
    ...
    $.contextMenu({
        ...
        callback: function(key, options) {
            if (key == 'add') {
                ...
                $('#modal-board-add').prop('store-type', 'google');
...
$('.google-product-import-btn').click(function (e) {
    e.preventDefault();

    $('#modal-google-product .google-store').val($(this).attr('store'));
    $('#modal-google-product').modal('show');
});

window.GoogleProductSelected = function (store, googleId) {
    $('#modal-google-product').modal('hide');

    $('#modal-supplier-link').prop('google-store', $('#modal-google-product .google-store').val());
    $('#modal-supplier-link').prop('google-product', googleId);
...
$('.add-supplier-info-btn').click(function (e) {
    ...
    $.ajax({
            ...
            setTimeout(function() {
                window.location.href = '/google/product/' + data.product;
            }, 1500);
        }).fail(function(data) {
