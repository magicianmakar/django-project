...
$('#fullfill-order-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Manual Fulfillment', 'Google', sub_conf.shop);

    $.ajax({
        url: api_url('fulfill-order', 'google'),
        type: 'POST',
...
$('.filter-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Filter Toggle', 'Google', sub_conf.shop);
...
$('.save-filter-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Save Filter', 'Google', sub_conf.shop);

    $.ajax({
        url: api_url('save-orders-filter', 'google'),
        type: 'POST',
...
function confirmDeleteOrderID(e) {
    ...
    function(isConfirm) {
        if (isConfirm) {
            ...
            ga('clientTracker.send', 'event', 'Delete Order ID', 'Google', sub_conf.shop);
        }
    });
}
...
function deleteOrderID(tr_parent, order_id, line_id) {
    ...
    function(isConfirm) {
        if (isConfirm) {
            $.ajax({
                url: api_url('order-fulfill', 'google') + '?' + $.param({'order_id': order_id, 'line_id': line_id, }),
                type: 'DELETE',
...
function addOrderSourceRequest(data_api, callback) {
    ...
    $.ajax({
        url: api_url('order-fulfill', 'google'),
        type: 'POST',
        data: data_api,
        context: {
            data_api: data_api
...
$('.note-panel .note-edit-save').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Edit Order Note', 'Google', sub_conf.shop);

    $.ajax({
        url: api_url('order-note', 'google'),
        type: 'POST',
        data: {
            'order_id': order_id,
            'store': store,
        ...
        success: function (data) {
            if (data.status === 'ok') {
                toastr.success('Order Note', 'Order note saved in Google store.');
...
$('.hide-ordered-btn').click(function () {
    ...
    ga('clientTracker.send', 'event', 'Hide Ordered Click', 'Google', sub_conf.shop);
});
...
$('.hide-non-connected-btn').click(function () {
    ...
    ga('clientTracker.send', 'event', 'Hide Non-Connected Click', 'Google', sub_conf.shop);
});
...
$('.add-supplier-btn').click(function (e) {
    ...
    ga('clientTracker.send', 'event', 'Order Add Supplier', 'Google', sub_conf.shop);

    var modalSupplierLinkEl = $('#modal-supplier-link');

    modalSupplierLinkEl.prop('google-store', $(this).attr('store-id'));
    modalSupplierLinkEl.prop('google-product', $(this).attr('google-product'));
    modalSupplierLinkEl.prop('google-product-title', $(this).attr('google-product-title'));
    modalSupplierLinkEl.prop('google-product-price', $(this).attr('google-product-price'));
    modalSupplierLinkEl.prop('google-product-image', $(this).attr('google-product-image'));
    modalSupplierLinkEl.prop('google-product-attributes', $(this).attr('google-product-attributes'));
...
$('.add-supplier-info-btn').click(function (e) {
    ...
    var data = {
        ...
        'product': modalSupplierLink.prop('google-product'),
        'product-title': modalSupplierLink.prop('google-product-title'),
        'product-price': modalSupplierLink.prop('google-product-price'),
        'product-image': modalSupplierLink.prop('google-product-image'),
        'product-attributes': modalSupplierLink.prop('google-product-attributes'),
        'google-store': modalSupplierLink.prop('google-store'),
    };
...
function pusherSub() {
    ...
    var getOrderNotes = function() {
        ...
        if (param.order_ids.length > 0) {
            ...
            return $.get(api_url('order-notes', 'google') + '?' + $.param(param));
        }
    };
