...
$(document).ready(function() {
    ...
    $('.add-board-btn').click(function(e) {
        e.preventDefault();
        $('#google-modal-board-add').modal('show');
    });

    $("#google-new-board-add-form").submit(function(e) {
        e.preventDefault();
        var title = $(this).find('input[name="title"]');
        var boardName = title.val().trim();

        $.ajax({
            url: api_url('boards-add', 'google'),
            type: 'POST',
            data: {title: boardName},
            success: function(data) {
                if ('status' in data && data.status === 'ok') {
                    $('#google-modal-board-add').modal('hide');
                    title.val('');
     ...
     $('.board-edit').on('click', function(e) {
        ...
        $.get(api_url('board-config', 'google'), {'board_id': boardId}).done(function(data) {
            var $form = $('#google-board-update-form');
            ...
            $('#google-modal-board-update').modal('show');
        });
    });

    $('#google-board-update-form').submit(function(e) {
        e.preventDefault();
        var data = {
        ...
        $.post(api_url('board-config', 'google'), data).done(function() {
            $('#google-modal-board-update').modal('hide');
    ...
    $('.board-delete').on('click', function(e) {
        e.preventDefault();
        var boardId = $(this).attr('data-board-id');
        var pageLocation = $(this).attr('data-page-location');
        var btn = $(this);
        ...
        swal({
            ...
            $.ajax({
                url: api_url('board', 'google') + '?' + $.param({board_id: boardId}),
                method: 'DELETE',
                success: function(data) {
                    if ('status' in data && data.status === 'ok') {
                        swal.close();
                        toastr.success('Board has been deleted.', 'Delete Board');
                        if (pageLocation === 'boards-list') {
                            btn.parents('.board-box').remove();
                        } else if (pageLocation === 'board-details') {
                            setTimeout(function() {
                                window.location.href = window.location.origin + '/google/boards/list';
                            }, 1000);
                        }
                    } else {
    ...
    $('.board-empty').on('click', function(e) {
        ...
        swal({
            ...
            $.ajax({
                url: api_url('board-empty', 'google'),
    ...
    $('.apply-btn').click(function(e) {
        ...
        if (action === 'delete') {
                ...
                if (isConfirmed) {
                   boardBox.find('input.item-select[type=checkbox]').each(function(i, el) {
                        if (el.checked) {
                            ...
                            if (action === 'delete') {
                                $.ajax({
                                    url: api_url('product', 'google') + '?' + $.param({product: product}),
                                    type: 'DELETE',
                                    success: function(data) {
            ...
            return;
        } else if (action === 'google-send') {
            currentBoardBox = boardBox;
            $('#modal-google-send').modal('show');
            return;
            ...
            $.ajax({
                url: api_url('board-products', 'google') + '?' + $.param(param),
                type: 'DELETE',
                success: function(data) {
                    if ('status' in data && data.status === 'ok') {
    ...
    $('#modal-products-edit-form #save-changes').click(function(e) {
        ...
        $.ajax({
            url: api_url('product-edit', 'google'),
            type: 'POST',
            data: data,
            success: function(data) {
                if ('status' in data && data.status === 'ok') {
    ...
    $('#google-send-btn').click(function(e) {
        ...
        channel.bind('pusher:subscription_succeeded', function() {
            $.ajax({
                url: api_url('products-info', 'google'),
                type: 'GET',
                ...
                success: function(data) {
                    ...
                    for (i = 0, len = productIds.length; i < len; i++) {
                        ...
                        sendProductToGoogle(productId, storeId, publish);
                    }
                }
            });
        });
        ...
        channel.bind('product-export', function(data) {
            ...
            if ($.inArray(productId, productIds) >= 0) {
                if (data.success) {
                    toastr.success('Product sent to the Google store.');
                    totalSuccess += 1;
                } else if (data.error) {
