...
def index(request):
    ...
    try:
        ...
        limit = safeInt(request.GET.get('limit'), 31)
        current_page = safeInt(request.GET.get('page'), 1)
...
def facebook_insights(request):
    ...
    if request.method == 'POST':
        ...
        if facebook_access_id:
            ...
            expires_in = safeInt(request.POST.get('fb_expires_in'))
...
def facebook_accounts(request):
    ...
    expires_in = safeInt(request.GET.get('fb_expires_in'))
...
def profit_details(request):
    ...
    limit = safeInt(request.GET.get('limit'), 20)
    current_page = safeInt(request.GET.get('page'), 1)
