...
function connect(store, fbProduct, variantsCount, csvIndexPosition) {
    var modal = $('#modal-supplier-link');
    modal.prop('fb-store', store);
    modal.prop('fb-product', fbProduct);
    modal.prop('fb-product-variants-count', variantsCount);
    modal.prop('fb-csv-index-position', csvIndexPosition);
    modal.prop('reload', 'true');

    modal.modal('show');
}

Vue.component('fb-imported-products-table', {
    template: '#fb-imported-products-table-tpl',
    props: ['task'],
    data: function () {
        return {
            task_id: null,
    ...
    methods: {
        ...
        addConnectSupplierButtonListener: function() {
            ...
            $('.add-supplier-info-btn').click(function (e) {
                ...
                var task_id;
                var product_id = modal.prop('fb-product');
                var vendor_name = $('.product-supplier-name').val();
                var supplier_url = $('.product-original-link').val();

                channel.bind('fb-product-import-completed', function(eventData) {
                ...
                channel.bind('pusher:subscription_succeeded', function() {
                     $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            store: modal.prop('fb-store'),
                            supplier: supplier_url,
                            vendor_name: vendor_name,
                            vendor_url: $('.product-supplier-link').val(),
                            product: product_id,
                            variants_count: modal.prop('fb-product-variants-count'),
                            csv_index_position: modal.prop('fb-csv-index-position'),
                        },
                    }).done(function (data) {
                        task_id = data.task;
                    }).fail(function(data) {
        ...
        disconnect: function(store_id, product_id) {
            ...
            function(isConfirmed) {
                ...
                if (isConfirmed) {
                    ...
                    channel.bind('pusher:subscription_succeeded', function() {
                        $.ajax({
                            url: api_url('disconnect-product', 'fb'),
                            data: {
                                product: product_id,
                                store: store_id,
                            },
        ...
        sync: function(store_id, product_id, variants_count, csv_index) {
            ...
            channel.bind('fb-product-sync-completed', function(eventData) {
                if (eventData.product === product_id) {
                    pusher.unsubscribe(channel);
                    btn.bootstrapBtn('reset');
                    btn.removeClass('disabled');
            ...
            channel.bind('pusher:subscription_succeeded', function() {
                $.ajax({
                    url: api_url('update-product-with-import', 'fb'),
                    data: {
                        store: store_id,
                        product: product_id,
        ...
        pusherSub: function() {
            ...
            $.ajax({
                url: api_url('import-product-options', 'fb'),
                type: 'GET',
                data: user_filter,
                success: function(data) {
                ...
                error: function(data) {
                    displayAjaxError('Facebook Import Products', data);
                }
            });

            window.channel.bind('fb-import-products-found', function(data) {
                if (vm.task_id && vm.task_id === data.task) {
                    ...
                    $.ajax({
                        url: api_url('import-products-from-cache', 'fb'),
                        data: {
...
if (typeof(Pusher) !== 'undefined') {
    ...
    window.channel.bind('pusher:subscription_succeeded', function(data){
        vue.$refs.fbImportedProductsTable.pusherSub();
    });
} else {
    toastr.error('This could be due to using Adblocker extensions<br>' +
