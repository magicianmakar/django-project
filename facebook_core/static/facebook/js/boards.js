...
$(document).ready(function() {
    ...
    $('.add-board-btn').click(function(e) {
        e.preventDefault();
        $('#fb-modal-board-add').modal('show');
    });

    $("#fb-new-board-add-form").submit(function(e) {
        e.preventDefault();
        var title = $(this).find('input[name="title"]');
        var boardName = title.val().trim();

        $.ajax({
            url: api_url('boards-add', 'fb'),
            type: 'POST',
            data: {title: boardName},
            success: function(data) {
                if ('status' in data && data.status === 'ok') {
                    $('#fb-modal-board-add').modal('hide');
                    title.val('');
     ...
     $('.board-edit').on('click', function(e) {
        ...
        $.get(api_url('board-config', 'fb'), {'board_id': boardId}).done(function(data) {
            var $form = $('#fb-board-update-form');
            ...
            $('#fb-modal-board-update').modal('show');
        });
    });

    $('#fb-board-update-form').submit(function(e) {
        e.preventDefault();
        var data = {
        ...
        $.post(api_url('board-config', 'fb'), data).done(function() {
            $('#fb-modal-board-update').modal('hide');
    ...
    $('.board-delete').on('click', function(e) {
        ...
        swal({
            ...
            $.ajax({
                url: api_url('board', 'fb') + '?' + $.param({board_id: boardId}),
                method: 'DELETE',
                success: function(data) {
                    if ('status' in data && data.status === 'ok') {
                        swal.close();
                        ...
                        if (pageLocation === 'boards-list') {
                            ...
                            setTimeout(function() {
                                window.location.href = window.location.origin + '/fb/boards/list';
                            }, 1000);
                        }
                    } else {
    ...
    $('.apply-btn').click(function(e) {
        ...
        if (action === 'delete') {
                ...
                if (isConfirmed) {
                   boardBox.find('input.item-select[type=checkbox]').each(function(i, el) {
                        if (el.checked) {
                            ...
                            if (action === 'delete') {
                                $.ajax({
                                    url: api_url('product', 'fb') + '?' + $.param({product: product}),
                                    type: 'DELETE',
                                    success: function(data) {
            ...
            $.ajax({
                url: api_url('board-products', 'fb') + '?' + $.param(param),
                type: 'DELETE',
                success: function(data) {
                    if ('status' in data && data.status === 'ok') {
    ...
    $('#fb-send-btn').click(function(e) {
        ...
        channel.bind('product-export', function(data) {
            ...
            if ($.inArray(productId, productIds) >= 0) {
                if (data.success) {
                    toastr.success('Product sent to the Facebook store.');
                    totalSuccess += 1;
                } else if (data.error) {
