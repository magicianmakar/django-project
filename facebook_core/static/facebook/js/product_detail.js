...
$("a.add-new-variant").click(function (e) {
    ...
    $('#fb-variants tbody').append(row);
    $('#fb-variants-panel-container').animate({ scrollTop: $('#fb-variants-panel-container').prop('scrollHeight')}, 1000);
});

$('body').on('click', 'tr.fb-variant td.add-variant-image', function(e) {
    e.preventDefault();
...
$('#modal-add-variant-image').on('click', '.add-variant-image-block', function(e) {
    ...
    var img = $('#fb-variants tr[variant-id="' + id + '"]').find('.add-variant-image img');
...
$('body').on('click', 'tr.fb-variant .delete-variant', function(e) {
    e.preventDefault();
...
$('body').on('click', 'tr.fb-variant .save-variant-name', function(e) {
    e.preventDefault();
...
window.multichannelProductSelected = function (master_id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if ('product' in data) {
                var channel_event = 'fb-product-update';
