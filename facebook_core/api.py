...
class FBStoreApi(ApiBase):
    ...
    def post_product_update(self, request, user, data):
        ...
        update_product_limit_check, product_limit_check, logs_count = FBUtils(user).check_product_update_limit(
    ...
    def get_import_products_status(self, request, user, data):
        ...
        try:
            result = FBUtils(user).get_import_products_status(store=store)
    ...
    def post_new_products_import_job(self, request, user, data):
        ...
        try:
            result = FBUtils(user).start_new_products_import_job(store=store)
        except:
            return self.api_error('Something went wrong. Please try again later.')
    ...
    def delete_disconnect_product(self, request, user, data):
        ...
        try:
            product = FBProduct.objects.get(guid=product_guid, user=user.models_user)
            permissions.user_can_delete(user, product)
        except FBProduct.DoesNotExist:
            return self.api_error('Product not found', status=404)
