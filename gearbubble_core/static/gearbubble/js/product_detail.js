...
function productExport(btn) {
    ...
    if (!store_id || store_id.length === 0) {
        swal('Product Export', 'Please choose a GearBubble store first!', 'error');
        return;
    }
    ...
    $.ajax({
        url: api_url('product-export', 'gear'),
        type: 'POST',
        data: {
        ...
        success: function (data) {
            ...
            channel.bind('product-export', function(data) {
                ...
                if (data.product == config.product_id) {
                    ...
                    if (data.success) {
                        toastr.success('Product Exported.','GearBubble Export');

                        setTimeout(function () {
                            window.location.reload(true);
                        }, 1500);
                    } else {
                        displayAjaxError('GearBubble Export', data);
                    }
                }
            });
        },
...
$('#product-update-btn').click(function (e) {
    ...
    $.ajax({
        url: api_url('product-update', 'gear'),
        type: 'POST',
        data: JSON.stringify ({
            'product': config.product_id,
        ...
        success: function (data) {
            ...
            channel.bind('product-update', function(data) {
                ...
                if (data.product == config.product_id) {
                    ...
                    if (data.success) {
                        toastr.success('Product Updated.','GearBubble Update');
                        setTimeout(function () {
                            window.location.reload(true);
                        }, 1500);
                    }
...
$('#modal-pick-variant .dropdown-menu li a').click(function(e) {
    ...
    if (config.gearbubble_options === null) {
        ...
        targetVariants = config.gearbubble_options.filter(function(o) { return o.name === val; });
        if (targetVariants.length > 0) {
...
$('#modal-pick-variant .btn-submit').click(function(e) {
    ...
    $.ajax({
        url: api_url('product-split-variants', 'gear'),
        type: 'POST',
        data: {
            product: product_id,
...
$('#duplicate-btn').click(function (e) {
    ...
    $.ajax({
        url: api_url('product-duplicate', 'gear'),
        type: 'POST',
        data: {
            product: product_id
        },
...
$('#save-product-notes').click(function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: api_url('product-notes', 'gear'),
        context: btn,
        data: {
...
function bindExportEvents(target) {
    ...
    $('.export-default-btn', target).click(function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: api_url('supplier-default', 'gear'),
            context: btn,
            data: {
                'product': form.data('product-id'),
                'export': form.data('export-id'),
    ...
    $('.export-delete-btn', target).click(function (e) {
        ...
        function(isConfirm) {
            if (isConfirm) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: api_url('supplier', 'gear') + '?' + $.param({
                        'product': form.data('product-id'),
...
$('#download-images').on('click', function(e) {
    ...
    $.ajax({
        url: api_url('product-image-download', 'gear'),
        type: 'GET',
        data: {
...
window.gearbubbleProductSelected = function (store, gear_id) {
    $.ajax({
        url: api_url('product-connect', 'gear'),
        type: 'POST',
        data: {
            product: config.product_id,
            gearbubble: gear_id,
        ...
        success: function (data) {
            $('#modal-gearbubble-product').modal('hide');
            window.location.hash = 'connections';
            window.location.reload();
        },
        error: function (data) {
...
$('.product-connection-change').click(function (e) {
    ...
    $('#modal-gearbubble-product').modal('show');
});
...
$('.product-connection-disconnect').click(function (e) {
    ...
    function(isConfirmed) {
        if (isConfirmed) {
            $.ajax({
                url: api_url('product-connect', 'gear') + '?' + $.param({
                    product: config.product_id,
                }),
                type: 'DELETE',
                success: function (data) {
