...
$('#board-product-send').click(function(e) {
    ...
    $.ajax({
        url: api_url('board-add-products', 'gear'),
        type: 'POST',
        data: data,
        success: function(data) {
            if ('status' in data && data.status == 'ok') {
...
function changeBoard(board_id, options) {
    ...
    $.ajax({
        url: api_url('product-board', 'gear'),
        type: 'POST',
        data: {
    ...
    $.contextMenu({
        ...
        callback: function(key, options) {
            if (key == 'add') {
                ...
                $('#modal-board-add').prop('store-type', 'gear');
...
$('.gearbubble-product-import-btn').click(function (e) {
    e.preventDefault();

    $('#modal-gearbubble-product .gearbubble-store').val($(this).attr('store'));
...
window.gearbubbleProductSelected = function (store, gear_id) {
    ...
    $('#modal-supplier-link').prop('gearbubble-store', $('#modal-gearbubble-product .gearbubble-store').val());
    $('#modal-supplier-link').prop('gearbubble-product', gear_id);
...
$('.add-supplier-info-btn').click(function (e) {
    ...
    $.ajax({
            ...
            setTimeout(function() {
                window.location.href = '/gear/product/' + data.product;
            }, 1500);
        }).fail(function(data) {
